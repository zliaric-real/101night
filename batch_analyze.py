...
# ══════════════════════════════════════════════════════════
#  手动解析 .mff 元数据 (绕过 MNE EGI reader 的 numpy 兼容问题)
# ══════════════════════════════════════════════════════════
def _read_mff_metadata(mff_path_str):
    """Parse .mff metadata from sensorLayout.xml + info1.xml + signal1.bin.

    EGI .mff: sensorLayout.xml defines all sensors, but signal1.bin only
    stores a subset (260 for EGI 256). MNE auto-generates E1-E256 from
    sensor numbers 1-256. We replicate that, plus detect the actual n_channels
    from file size divisibility.

    Returns: {ch_names: [str], n_channels: int, sfreq: float, n_times: int}
    """
    import xml.etree.ElementTree as ET

    mff = Path(mff_path_str)

    # ── Parse sensorLayout.xml → channel names ──
    ...
    # ── Determine actual n_channels from signal1.bin ──
    # sensorLayout may have 269 entries but signal1.bin only has 260.
    # Find n where file_size % (n * 4) is minimal (ideally 0).
    signal_path = mff / "signal1.bin"
    file_size = signal_path.stat().st_size
    total_values = file_size // 4

    # Try common EGI channel counts: 256, 257, 260, 269
    # EGI 256 has 256 EEG + VREF + some aux. MNE reports 260 for this dataset.
    n_channels = 260
    # If the expected n doesn't divide cleanly, try alternatives
    ...
    # If n_channels doesn't match len(channels), truncate/pad channel names
    ...
    # ── sfreq: EGI 256 hardware is fixed at 250 Hz ──
    # info1.xml <sampRate> is the authoritative source when present.
    # epochs.xml duration ÷ n_times is UNRELIABLE because:
    #   (a) n_times depends on n_channels detection (260 vs 256 vs 269)
    #       and a 1-channel error cascades into wrong sfreq
    #   (b) epochs.xml may not span the full recording
    # Observed failures: sfreq=721, 1386, 998, 1990, 2143, etc.
    # For the 101-nights EGI 256 dataset, always force 250 Hz as the
    # ground truth; info1.xml <sampRate> overrides this only if it
    # explicitly reports a different valid value.
    sfreq = 250.0
